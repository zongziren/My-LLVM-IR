...
void IRBuilder::visit(SyntaxTree::UnaryExpr &node)
{
  ...
  if (node.op == SyntaxTree::UnaryOp::MINUS)
  {
    ...
    if (tmp_val->get_type()->is_integer_type() && dynamic_cast<IntegerType *>(tmp_val->get_type())->get_num_bits() == 32)
    //是32位整数
    {
      auto r_val = tmp_val;
    ...
    else if (tmp_val->get_type()->is_integer_type() && dynamic_cast<IntegerType *>(tmp_val->get_type())->get_num_bits() == 1)
    //是1位整数
    {
...
void IRBuilder::visit(SyntaxTree::IfStmt &node)
{
  ...
  {
    if (ret_val->get_type()->is_integer_type() && dynamic_cast<IntegerType *>(tmp_val->get_type())->get_num_bits() == 32)
      cond_val_int = builder->create_icmp_ne(tmp_val, CONST_INT(0));
    else if (ret_val->get_type()->is_integer_type() && dynamic_cast<IntegerType *>(tmp_val->get_type())->get_num_bits() == 1)
